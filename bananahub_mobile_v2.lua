...
-- === BEGIN: WZAGRL bring integration (mobile-compatible helpers) ===
local TweenService = game:GetService("TweenService")
local Players = game:GetService("Players")
local LocalPlayer = Players.LocalPlayer

-- adjustable settings
local bringRange = 150
local flySpeed = 350
...
local function isRealEnemy(enemy)
    ...
    if hum.Health <= 0 or hum.Health > (hum.MaxHealth or math.huge) then return false end
    if workspace:FindFirstChild(\"Enemies\") and not enemy:IsDescendantOf(workspace.Enemies) then return false end
    return true
end
...
local function getNearestEnemy()
    ...
    for _, mob in pairs(workspace.Enemies:GetChildren()) do
        if isRealEnemy(mob) then
            local mhrp = mob:FindFirstChild(\"HumanoidRootPart\")
            if mhrp then
                local d = (mhrp.Position - hrp.Position).Magnitude
                if d < minDist and d <= bringRange then
                    closest = mob
                    minDist = d
                end
            end
...
local function bringEnemies(target)
    ...
    for _, mob in pairs(workspace.Enemies:GetChildren()) do
        if mob.Name == target.Name and isRealEnemy(mob) then
                ...
                if dist <= bringRange then
                    task.spawn(function()
                        pcall(function()
                            ...
                            local duration = (hrp.Position - targetHRP.Position).Magnitude / math.max(1, flySpeed)
                            local tween = TweenService:Create(hrp, TweenInfo.new(duration, Enum.EasingStyle.Linear), {CFrame = targetHRP.CFrame})
                            tween:Play()
                        end)
                    end)
                end
                        ...
                        if (v1433:FindFirstChild("Humanoid") and v1433:FindFirstChild("HumanoidRootPart") and (v1433.Humanoid.Health > 0)) then
                            if (v1433.Name == Ms) then
                                repeat
                                    ...
                                    AttackNoCoolDown();
                                    bringmob = true
local __wzag_target = getNearestEnemy()
if __wzag_target then
    pcall(function()
        bringEnemies(__wzag_target)
        pcall(function() FarmPos = __wzag_target.HumanoidRootPart.CFrame end)
        pcall(function() MonFarm = __wzag_target.Name end)
    end)
end;
                                    AutoHaki();
...
-- ============================
-- BỔ SUNG: Bring logic v2
-- Tác giả: ChatGPT (tùy chỉnh theo yêu cầu người dùng)
-- Mục đích:
--  - bringRange mặc định = 350 (có thể chỉnh bằng _G.BringRange trong phần cài đặt của BananaHub)
--  - bringSpeed cố định = 300 (không thể chỉnh)
--  - Nếu _G.FarmNearest == true => bring TẤT CẢ quái thật trong phạm vi bringRange (không phân biệt loại)
--  - Nếu _G.FarmNearest == false => behavior như cũ: bring quái cùng loại (nếu code cũ yêu cầu)
--  - Hàm này an toàn cho mobile (pcall quanh các thao tác có thể lỗi)
-- ============================

-- Tham số cấu hình
_G.BringRange = _G.BringRange or nil  -- nếu BananaHub đã có setting, giữ; nếu không sẽ dùng mặc định bên dưới
local bringRange = (_G.BringRange and tonumber(_G.BringRange)) or 350  -- default 350, có thể ghi đè bằng _G.BringRange
local bringSpeed = 300  -- cố định, không cho phép thay đổi

local TweenService = game:GetService("TweenService")
local Players = game:GetService("Players")
local LocalPlayer = Players.LocalPlayer

-- Kiểm tra quái thật (anti-fake) - an toàn
function isRealEnemy_override(enemy)
    if not enemy then return false end
    if type(enemy) ~= "table" and type(enemy) ~= "userdata" then return false end
    local ok, hrp = pcall(function() return enemy:FindFirstChild("HumanoidRootPart") end)
    local ok2, hum = pcall(function() return enemy:FindFirstChild("Humanoid") end)
    if not ok or not ok2 or not hrp or not hum then return false end
    local health = 0
    local maxh = math.huge
    pcall(function() health = hum.Health end)
    pcall(function() maxh = hum.MaxHealth end)
    if health <= 0 or health > (maxh or math.huge) then return false end
    -- Nếu workspace.Enemies tồn tại thì chỉ dùng những model nằm trong đó
...
-- Tìm quái gần nhất trong bringRange (dùng để lựa target nếu cần)
function getNearestEnemy_override()
    local success, result = pcall(function()
        ...
        local closest, minDist = nil, math.huge
        for _, mob in pairs(workspace.Enemies:GetChildren()) do
            if isRealEnemy_override(mob) then
                local mhrp = mob:FindFirstChild("HumanoidRootPart")
                if mhrp then
                    local d = (mhrp.Position - hrp.Position).Magnitude
                    if d < minDist and d <= bringRange then
...
-- Bring logic chính
-- Nếu _G.FarmNearest == true => bring TẤT CẢ quái trong bringRange (bất kể tên)
-- Ngược lại => bring chỉ quái cùng loại (tương thích với logic cũ)
function bringEnemies_override(target)
    pcall(function()
        if not target then return end
        if not isRealEnemy_override(target) then return end
        -- cập nhật bringRange nếu _G.BringRange thay đổi runtime
        if _G.BringRange and tonumber(_G.BringRange) then
            bringRange = tonumber(_G.BringRange)
        end
        local char = LocalPlayer.Character
        if not char or not char:FindFirstChild("HumanoidRootPart") then return end
        local myPos = char.HumanoidRootPart.Position

        -- nếu đang ở chế độ FarmNearest -> gom TẤT CẢ quái thật trong phạm vi
        if _G.FarmNearest then
                ...
                if isRealEnemy_override(mob) then
                    ...
                    if hrp then
                        ...
                        if dist <= bringRange then
                            task.spawn(function()
                                pcall(function()
                                    -- đảm bảo không vướng vật lý khi tween
                                    hrp.CanCollide = false
                                    local duration = (hrp.Position - target.HumanoidRootPart.Position).Magnitude / math.max(1, bringSpeed)
                                    local tween = TweenService:Create(hrp, TweenInfo.new(duration, Enum.EasingStyle.Linear), {CFrame = target.HumanoidRootPart.CFrame})
                                    tween:Play()
                                end)
                            end)
                        end
            ...
            -- chế độ cũ: chỉ gom quái cùng loại với target (tương thích)
            local targetName = target.Name
            for _, mob in pairs(workspace.Enemies:GetChildren()) do
                if isRealEnemy_override(mob) and mob.Name == targetName then
                    local hrp = mob:FindFirstChild("HumanoidRootPart")
                    if hrp then
                        local dist = (hrp.Position - myPos).Magnitude
                        if dist <= bringRange then
...
-- Ghi đè tên hàm cũ (nếu file gốc gọi những tên này, bây giờ sẽ dùng hàm override)
isRealEnemy = isRealEnemy_override
getNearestEnemy = getNearestEnemy_override
bringEnemies = bringEnemies_override

-- Chú thích:
-- • Nếu BananaHub của bạn thay tên biến (ví dụ gọi function gốc khác tên), bạn có thể sửa tên hàm ở trên cho khớp.
-- • bringRange có thể được thay bằng _G.BringRange bất kỳ lúc nào; setting gốc của BananaHub nếu có, hãy liên kết giá trị đó tới _G.BringRange.
-- • bringSpeed cố định = 300 theo yêu cầu.

-- KẾT THÚC: Bring logic v2
